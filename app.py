...
# ==========================================
# CORREÇÃO 1: st.secrets não tem método .get()
# Usar acesso direto com try/except
# ==========================================
try:
...
def carregar_do_github(nome_arquivo):
    ...
    if resposta.status_code == 200:
        ...
        try:
            return pd.read_excel(io.BytesIO(conteudo_bin)), dados["sha"]
        except Exception:
            return None, None
    return None, None
...
def salvar_no_github(df, nome_arquivo, sha=None):
    ...
    # CORREÇÃO 2: BytesIO precisa ser usado diretamente no ExcelWriter
    buffer = io.BytesIO()
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        df.to_excel(writer, index=False)
    buffer.seek(0)  # <- essencial para não enviar buffer vazio
...
if file_clientes and file_agendamentos:
    ...
    col_servico = next(
        (c for c in df_novos_agendamentos.columns if 'servi' in c.lower() or 'proced' in c.lower()),
        None
    )
    if col_servico:
        df_novos_agendamentos.rename(columns={col_servico: 'Serviço(s)'}, inplace=True)

    # Garante que a coluna Serviço(s) existe mesmo que não encontrada
    if 'Serviço(s)' not in df_novos_agendamentos.columns:
        df_novos_agendamentos['Serviço(s)'] = ''
    ...
    coluna_niver = next(
        (c for c in df_novas_clientes.columns
    ...
    if not clientes_sem_niver.empty:
        ...
        if sel_cli != "Pular...":
            dat_in = st.sidebar.text_input(
                f"Data para {sel_cli.split()[0]} (DD/MM):", max_chars=5
            )
            if st.sidebar.button("Gravar Data"):
                ...
                if match_dt and int(match_dt.group(2)) <= 12:
                    ...
                    # CORREÇÃO 3: compatibilidade com versões antigas do Streamlit
                    try:
                        st.rerun()
    ...
    df_ultimos = df_acumulado_agendamentos.groupby(
        ['Cliente', 'Serviço(s)'], as_index=False
    )['Data'].max()
    ...
    # CORREÇÃO 4: concat seguro mesmo com lista vazia
        ...
        if len(token_nv) == 2:
            try:
                if int(token_nv[0]) == dt_alvo.day and int(token_nv[1]) == dt_alvo.month:
                    lista_aniv.append({
                        'Cliente': lin['Nome'],
                        'Serviço(s)': 'Aniversário Especial',
                        'Última Visita': '-',
                        'Data Ideal Retorno': dt_alvo.strftime('%d/%m/%Y'),
                        'Dias de Atraso': 999,
                        'Tipo de Gatilho': 'Aniversário',
                    })
            except (ValueError, TypeError):
                continue

    # CORREÇÃO 5: concat robusto — garante colunas compatíveis em listas vazias
    ...
    if not df_unificado.empty:
        ...
        df_final = pd.merge(
            df_unificado,
            df_acumulado_clientes,
            left_on='Cliente',
            right_on='Nome',
            how='inner',
        )

        # Remove colunas sensíveis
        ...
        # CORREÇÃO 6: garante que coluna Telefone existe antes de usá-la
        if 'Telefone' not in df_final.columns:
            df_final['Telefone'] = ''

        st.markdown(
            "<br><h3 style='color: #FFC0CB;'>📱 Painel de Controle de Abordagens</h3>",
            unsafe_allow_html=True,
        )
        ...
        for _, linha in df_final.iterrows():
            ...
            # CORREÇÃO 7: tratamento robusto de telefone (pode vir como float)
            tel_raw = str(linha.get('Telefone', '')).strip()
            tel_raw = re.sub(r'[^\d]', '', tel_raw)  # remove tudo que não é dígito
            ...
            if linha['Tipo de Gatilho'] == 'Aniversário':
                ...
                b_lbl = f"⏳ {linha['Serviço(s)']}"
                atr_t = f"<span style='color: #ff4d4d; font-weight:bold;'>{linha['Dias de Atraso']} dias</span>"
                # CORREÇÃO 8: capitalização natural do serviço na mensagem
                servico_msg = linha['Serviço(s)'][0].lower() + linha['Serviço(s)'][1:]
                msg = (
                    f"Olá, {p_nome}! ✨ Notamos aqui no Sinfonia Hair que sua última {servico_msg} "
            ...
            st.markdown(
                f"""<div style="background-color: #1A1A1A; padding: 15px; border-left: 4px solid #FFC0CB;
